...
class SimpleNoteCreateChangeViewController : UIViewController, UITextViewDelegate {
    
    @IBOutlet weak var noteTitleTextField: UITextField!
    @IBOutlet weak var noteTextTextView: UITextView!
    @IBOutlet weak var noteDoneButton: UIButton!
    @IBOutlet weak var noteDateLabel: UILabel!
    @IBOutlet var edtNoOfDay: UITextField!
    ...
    private func addItem() -> Void {
        guard let days = edtNoOfDay.text else {
            return
        }
        let note = Zotes(noteTitle: noteTitleTextField.text!, noteText:  noteTextTextView.text!, noteTimeStamp: noteCreationTimeStamp, dayworked: 0, totalday: Int64((days as NSString).intValue))
        
        SimpleNoteStorage.storage.createNote(noteToBeAdded: note)
    ...
    private func changeItem() -> Void {
        ...
        if let changingReallySimpleNote = self.changingReallySimpleNote {
            ...
            let note = Zotes(noteId:  changingReallySimpleNote.noteId ,noteTitle: noteTitleTextField.text!, noteText:  noteTextTextView.text!, noteTimeStamp: noteCreationTimeStamp, dayworked: 0, totalday: Int64((days as NSString).intValue))
    ...
    override func viewDidLoad() {
        ...
        if let changingReallySimpleNote = self.changingReallySimpleNote {
            ...
            noteDateLabel.text = SimpleNoteDateHelper.convertDate(date: Date.init(seconds: noteCreationTimeStamp))
            noteTextTextView.text = changingReallySimpleNote.noteText
            noteTitleTextField.text = changingReallySimpleNote.noteTitle
            edtNoOfDay.text = "\(changingReallySimpleNote.totalday)"
